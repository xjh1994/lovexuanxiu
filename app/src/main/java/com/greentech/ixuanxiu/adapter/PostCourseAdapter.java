 ...
 */
public class PostCourseAdapter extends UltimateViewAdapter {
    ...
    public PostCourseAdapter(Context context, List<Post> postList) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (position < getItemCount() && (customHeaderView != null ? position <= postList.size() : position < postList.size()) && (customHeaderView != null ? position > 0 : true)) {
            ...
            if (null != course) {
                ((ViewHolder) holder).courseName.setText(course.getName());
            }
    ...
    public UltimateRecyclerviewViewHolder onCreateViewHolder(ViewGroup parent) {
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_post_course_list, parent, false);
